...
export default function SceneCard({
  ...
  return (
    <div className="bg-[#FFD977] border-6 border-azul-noche rounded-4xl p-4 flex flex-col gap-4 w-full max-w-sm mx-auto shadow-xl mt-10 transition-all duration-300 ease-in-out hover:-translate-y-3 hover:scale-[1.02] hover:shadow-2xl font-caveat-brush">
      <div className="bg-azul-noche rounded-2xl py-8 px-4 flex items-center justify-center">
        <h3 className="text-white text-3xl text-center font-bold tracking-wider">{title}</h3>
      </div>
      ...
      <div className="flex flex-1 justify-center flex-col gap-3 mt-2">
        {displayedItems.map((item, index) => (
          <div
            key={index}
            onClick={() => onItemClick ? onItemClick(item.title) : onActionClick?.()} 
            className={`${fixedColors[index]} flex justify-between items-center py-2 px-4 rounded-r-full -ml-4 pr-6 hover:-mr-2 transition-all cursor-pointer`}
          >
            <span className="text-2xl text-black truncate mr-2">{item.title}</span>
        ...
        {extraItemsCount > 0 && (
          <div className="text-left pr-6 -mt-1 cursor-pointer">
            <span className="text-azul-noche text-lg font-bold">+ {extraItemsCount} más...</span>
          </div>
        )}
      </div>
